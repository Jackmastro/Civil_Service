...
try:
    ...
    hx = HX711(
        dout_pin=16, pd_sck_pin=5, gain_channel_A=64, select_channel='A')
    ...
    input(
        'That is why I recommend always passing argument readings=20 or higher value (press ENTER)'
    )
    for i in range(40):
    ...
    print(f'Current offset for channel A and the current gain (64): {offset} (IMP: USE THIS VALUE IN THE SENSOR CLASS AND TEST)')
    # for channel B
    ...
    print(f'Current scale ratio is set to {current_ratio} (IMP: USE THIS VALUE IN THE SENSOR CLASS AND TEST)')
